...
public final class GodotPermissionManager extends Godot.SingletonBase {
	...
	static public Godot.SingletonBase initialize(Activity activity) {
		return new GodotPermissionManager(activity);
	}

	public GodotPermissionManager(Activity p_activity) {
		registerClass("PermissionManager", new String[] { "init", "requestPermission", "requestPermissions", "checkPermissionStatus", "getPermissionName" });
	...
	protected void onMainRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
		...
		if (REQUEST_ALL_PERMISSION_REQ_CODE != requestCode) {
			return;
		}
		
		int i = 0;
		while (i < permissions.length) {
			GodotLib.calldeferred(GodotPermissionManager.this.instance_id,
					"_on_permission_result", new Object[] { permissions[i], grantResults[i] == 0 });
			i++;
		}
	}
	...
	public boolean requestPermission(int p_permission) {
		if (!initialize) {
			return false;
		}
		...
		if (ContextCompat.checkSelfPermission(mActivity, requested) != PackageManager.PERMISSION_GRANTED) {
			mActivity.requestPermissions(new String[] { requested }, REQUEST_ALL_PERMISSION_REQ_CODE);
			return false;
		}
	...
	public boolean requestPermissions(int[] p_permissions) {
		if (!initialize) {
			return false;
		}
		...
		for (String manifestPermission : manifestPermissions) {
			...
			while (i < p_permissions.length) {
				...
				if (manifestPermission.equals(requested)) {
					boolean granted = ContextCompat.checkSelfPermission(mActivity, requested) == PackageManager.PERMISSION_GRANTED;
					if (!granted)
		...
		mActivity.requestPermissions(requestArray, REQUEST_ALL_PERMISSION_REQ_CODE);
		return false;
	}

	public boolean checkPermissionStatus(int p_index) {
		if (!initialize) {
			return false;
		}
		...
		if (requested != null) {
			return ContextCompat.checkSelfPermission(mActivity, requested) == PackageManager.PERMISSION_GRANTED;
		}

		return false;
    }
    
    public String getPermissionName(int p_key) {
		return permissionList.get(p_key);
	}
